import { credentials, authUsers, authToken } from '../../core/modules/auth/credentials.auth';
import { AuthCasesInterface } from '../../Infraestrucure/interfaces/routes.interfaces';
import { v4 as uuidv4 } from 'uuid';
import bcrypt from 'bcrypt';
import { contextAuth } from '../../core/modules/auth/context.auth';
...
export class AuthCases implements AuthCasesInterface {
   ...
   */
    ...
    async login(data: credentials): Promise<authToken | boolean> {
        if (!data.password || !data.username) {
            return false;
        }
        // =============== Control if user exists into context ======== //
